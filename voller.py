    ...
    while i < imax:
        try:
            ...
            if len(orders) != 0:
                ...
                time.sleep(1)
                bal = getShitcoinBalance(ticker_symbol)
                print ticker_symbol + ': Bought ' + str(bal) + ' @ ' + "%.8f" % o_price
                if bal > order_cutoff:
                    time.sleep(2)
...
def restartBottas():
    os.execl(sys.argv[0], '')
def getOneMinuteRatio(ticker_symbol):
    klines = client.get_historical_klines(ticker_symbol, Client.KLINE_INTERVAL_1MINUTE, "1 hour ago UTC")
    last_price = float(klines[-1][4])
    last_volume = float(klines[-1][5])

    price_5m_ago = float(klines[-5][4])
    price_15m_ago = float(klines[-15][4])
    price_30m_ago = float(klines[-30][4])
    price_60m_ago = float(klines[0][4])
    min5_in_percent = (last_price - price_5m_ago) / price_5m_ago * 100
    min15_in_percent = (last_price - price_15m_ago) / price_15m_ago * 100
    min30_in_percent = (last_price - price_30m_ago) / price_30m_ago * 100
    min60_in_percent = (last_price - price_60m_ago) / price_60m_ago * 100
    ...
    for k in klines:
        price_close = float(k[4])
        volume = float(k[5])
        price_total += price_close
        volume_total += volume
        
    price_avg = price_total / len(klines)
    volume_per_min = volume_total / len(klines)
    volume_btc = volume_total * price_avg
    one_min_ratio = last_volume / volume_per_min
    
    return last_price, volume_btc, one_min_ratio, min5_in_percent, min15_in_percent, min30_in_percent, min60_in_percent
...
if __name__ ==  '__main__':
    ...
    ticker_list = []
    for t in tickers:
        if t['symbol'].endswith('BTC') and not t['symbol'].startswith('BNB'):
            ticker = t['symbol']
    ...
    while True:
        try:
            ...
            buy_fraction = btc_ballpark
            
            for t in ticker_list:
                last_price, volume_btc, one_min_ratio, min5_in_percent, min15_in_percent, min30_in_percent, min60_in_percent = getOneMinuteRatio(t)
                price_str = '(' + "{0:.2f}%".format(min5_in_percent) + ', ' "{0:.2f}%".format(min15_in_percent) + ', ' "{0:.2f}%".format(min30_in_percent) + ', ' "{0:.2f}%".format(min60_in_percent) + ')'
                volume_str = "{0:.0f}".format(volume_btc)
                one_min_ratio_str = "{0:.2f}".format(one_min_ratio)
                if one_min_ratio > 1:
                    print t + ' '*(10-len(t)) + "{0:.8f}".format(last_price) + ' '*(8-len(one_min_ratio_str)) + one_min_ratio_str + ' '*(7-len(volume_str)) + volume_str + ' '*(40-len(price_str)) + price_str
            
                #if one_min_ratio > 8 and min5_in_percent > 0.5 and min5_in_percent < 2 and min15_in_percent > 0.5 and min15_in_percent < 1.5 and min30_in_percent < 2 and min60_in_percent < 3 and volume_btc > 40:
                    #bal_chk = getShitcoinBalance(t)
                    #if bal_chk > 1:
                        #restartBottas()
                    #os.system('play --no-show-progress --null --channels 1 synth %s sine %f' % (0.5, 440))
                    
                    #buy_price, order_amount, steps, ticksize, precision = buyShitcoin(t, last_price, loop_buy_style, buy_fraction)
                    #buys += 1
                    #spinOffHandler(t)
                    #print 'Buys: ' + str(buys)
                    #time.sleep(30)
