...
change_since_buy_profit_outset = 2
change_since_buy_profit_increment = 1
#change_since_buy_loss_outset = -1
#change_since_buy_loss_increment = -0.2
...
def positionHandler(ticker_symbol, buy_price, order_amount):
    ...
    while True:
        ...
        try:
            ...
            # stop loss
            #if change_since_buy < change_since_buy_loss_outset:
                #sellFraction(ticker_symbol, 3)
        
            #if change_since_buy < change_since_buy_loss_outset + change_since_buy_loss_increment:
                #sellFraction(ticker_symbol, 2)
        
            #if change_since_buy < change_since_buy_loss_outset + (change_since_buy_loss_increment * 2):
            ...
            if change_since_high < change_since_high_outset and chk_trail2 == False:
                chk_trail2 = True
                if chk_profit2 == True:
                    sellFraction(ticker_symbol, 1)
                    lastCheck(ticker_symbol)
            ...
            if change_since_high < change_since_high_outset + change_since_high_increment and chk_trail3 == False:
                chk_trail3 = True
                sellFraction(ticker_symbol, 1)
                lastCheck(ticker_symbol)
                return
            ...
            # take profit
            #if change_since_buy > change_since_buy_profit_outset and chk_profit1 == False:
                #chk_profit1 = True
                #if chk_trail2 == True:
                    #sellFraction(ticker_symbol, 1)
                    #lastCheck(ticker_symbol)
            ...
            if change_since_buy > change_since_buy_profit_outset and chk_profit2 == False:
                chk_profit2 = True
                if chk_trail2 == True:
                    sellFraction(ticker_symbol, 1)
                    lastCheck(ticker_symbol)
            ...
            if change_since_buy > change_since_buy_profit_outset + change_since_buy_profit_increment:
                chk_profit3 = True
                sellFraction(ticker_symbol, 1)
                lastCheck(ticker_symbol)
                return
            ...
            print '\n'
            while True:
                fraction_to_sell = int(raw_input('How much? (0: Continue, 66: Dump, 99: Balance): '))
                
                if fraction_to_sell == 0:
                    break
                if fraction_to_sell == 66:
...
def adjustAlgo(ticker_symbol):
    ...
    if algo_choice == 1:
        ...
        change_since_buy_profit_outset = 1.5
        change_since_buy_profit_increment = 0.5
    if algo_choice == 2:
        change_since_high_outset = -3
        change_since_high_increment = -0.3
        change_since_buy_profit_outset = 5
        change_since_buy_profit_increment = 2
    return
...
def sellFraction(ticker_symbol, sell_fraction):
    ...
    match_amt = ((order_amount * uniform(0.2, 0.35)) // steps) * steps
    ask_amt = ((order_amount - match_amt) // steps) * steps    
    
    print 'Match: ' + str(match_amt) + '  Ask: ' + str(ask_amt)
    

    
    placeNewAskSellOrder(ticker_symbol, steps, ticksize, precision, ask_amt)
    
    time.sleep(1)
    
    checkSellOrder(ticker_symbol, steps, ticksize, precision, ask_amt, 'ask', sell_fraction)
    placeNewMatchSellOrder(ticker_symbol, steps, ticksize, precision, match_amt)    
    

def checkSellOrder(ticker_symbol, steps, ticksize, precision, amt, sell_style, fraction):

    open_order = client.get_open_orders(symbol=ticker_symbol)
    exec_qty = 0
    while open_order != []:
        
        time.sleep(randint(10, 15)) # seconds order stays in book
        open_order = client.get_open_orders(symbol=ticker_symbol)
        try:
            ...
            if exec_qty == 0.0:
                ...
                time.sleep(randint(2, 4)) # seconds between cancel and place new order
                if sell_style == 'ask':
                    placeNewAskSellOrder(ticker_symbol, steps, ticksize, precision, amt)
                if sell_style == 'match':
                    placeNewMatchSellOrder(ticker_symbol, steps, ticksize, precision, amt)
            else:
                #fin_price, fin_qty = getLastTrade(ticker_symbol)
                print ticker_symbol + ': Sold something' # + str(fin_qty) + ' @ ' + '%.8f' % fin_price
                cancelOrder(ticker_symbol, order_id)
                new_order_amt = amt - exec_qty
                time.sleep(randint(2, 4)) # seconds between cancel and place new order
                if sell_style == 'ask':
                    placeNewAskSellOrder(ticker_symbol, steps, ticksize, precision, new_order_amt)
                if sell_style == 'match':
                    placeNewMatchSellOrder(ticker_symbol, steps, ticksize, precision, new_order_amt)
                amt = new_order_amt
        except KeyboardInterrupt:
            coin_balance = getShitcoinBalance(ticker_symbol)
            order_amount = (coin_balance // steps) * steps
...
def placeNewMatchSellOrder(ticker_symbol, steps, ticksize, precision, match_amt):
    
    match_bid = matchBid(ticker_symbol, match_amt)
    match_price_str = "{:0.0{}f}".format(match_bid, precision)
    print 'Match: ' + str(match_amt) + ' @ ' + match_price_str
    
    try:
        order = client.order_limit_sell(
        symbol=ticker_symbol,
        quantity=match_amt,
        price=match_price_str
        )
        order_str = ticker_symbol + ': Selling ' + str(match_amt) + ' @ ' + match_price_str + '...'
        print order_str
        return
    except:
        print 'Match sell order failed. Amount likely too low.'
        return

def placeNewAskSellOrder(ticker_symbol, steps, ticksize, precision, ask_amt):
    
    low_ask = findLowestAsk(ticker_symbol)
    ask_price_str = "{:0.0{}f}".format((low_ask - ticksize), precision)

    print 'Ask: ' + str(ask_amt) + ' @ ' + ask_price_str
    ...
    try:
        ...
        order_str = ticker_symbol + ': Selling ' + str(ask_amt) + ' @ ' + ask_price_str + '...'
        print order_str
        return
    except:
        print 'Ask sell order failed. Amount likely too low.'    
        lastCheck(ticker_symbol)
